...
\begin{document}
	...
	\end{proposition}
	\begin{Proof}
		Предположим, что $m=\tilde{m}$ и $s^{2} = \tilde{s}^{2}$, и получим систему \eqref{7}.
		
		$\Phi (y) = \mathsf{P}\left( \eta = \dfrac{\xi-m}{s}\leq y\right) $ "--- функция распределения стандартного нормального распределения, тогда система \eqref{6} записывается как
		\begin{equation}
			\begin{pmatrix} 1&1&1\\ 
...
При этом значение $\sigma$ одинаковое для любых пар i и j.
...
и результат не зависит от i.
	...
	\begin{Proof}
		\begin{align*}
			\hat{\F}(y) &= \mathsf{P}\left(\eta\leq y \right) = \mathsf{P}\left(\frac{\xi-m}{s} \leq y \right)= \\
	...
	\begin{alg}\label{al2}
		...
		\begin{enumerate}
			...
			\item Вычисляем значения $\hat{\F}^{-1}(\pi)$ для случайной величины $\eta$ по формуле (12).
	...
	\begin{ex}
		...
		Посчитаем пример для $\dfrac{x_{90}}{x_{10}}=3$. По формулам из этого раздела получаeм
		
		\[\sigma = \frac{\log\left( \dfrac{x_{90}}{x_{10}}\right) }{\phi ^{-1}(0,9)-\phi ^{-1}(0,1)}\approx 0.428626,\]
		
		\[\mathsf{\hat{F}}^{-1}(p)= \frac{\exp\left( \sigma \phi ^{-1}(p)-\dfrac{\sigma ^{2}}{2}\right) -1}{\sqrt{\exp(\sigma ^{2})-1}}\].
		\[\mathsf{\hat{F}}^{-1}(0.9) \approx 1.2915826424,\]
		\[\mathsf{\hat{F}}^{-1}(0.1)  \approx -1.0539640761,\]
		\[\mathsf{\hat{F}}^{-1}(0.5)  \approx -0.1954343914.\]
	...
	\begin{Proof}
		...
		\[\exp(\sigma^{2})+\exp(-\sigma^{2})-\exp\left( \dfrac{\sigma^{2}}{2}\right)\left(\exp(\Phi^{-1}(\pi)\sigma)+\exp(-\Phi^{-1}(\pi)\sigma)\right) \leq 0.\] 
		Рассмотрим 
		\[\exp(\Phi^{-1}(\pi)\sigma)+\exp(-\Phi^{-1}(\pi)\sigma).\]
		Эта сумма увеличивается при уменьшении $\pi$ и фиксированной $\sigma$, так как увеличивается по модулю значение $\Phi^{-1}(\pi)$.
		Значит, вычитаемое
		\[\exp\left( \dfrac{\sigma^{2}}{2}\right)\left(\exp(\Phi^{-1}(\pi)\sigma)+\exp(-\Phi^{-1}(\pi)\sigma)\right)\]
		становится все больше, а ограничение на $\sigma$ слабее.
	...
	\begin{proposition}\label{pr5}
		1. Ошибка аппроксимации мат. ожидания логнормального распределения по методу Свонсона, применяемому к нормальному распределению, равна
		...
		2. Ошибка аппроксимации дисперсии логнормального распределения по методу Свонсона, применяемому к нормальному распределению, равна
	...
	\end{proposition}
	\begin{Proof}
		1. Выразим ошибку аппроксимации мат. ожидания логнормального распределения через параметры $\mu$ и $\sigma$.
	...
	Видим, что при $\sigma\leq1.5$ ошибка аппроксимации мат.ожидания меньше $12\%$, а ошибка аппроксимации дисперсии может достигать $80\%$. 
	...
	\begin{alg}\label{al3}
		...
		\begin{enumerate}
			...
			По набору квантилей $\xi_{1}$ находим параметры $\mu_{1}$, $\sigma_{1}$ нормального распределения по формулам (12) и (13).
			\item $y_{\pi}$, $y_{0.5}$, $y_{1-\pi}$ $\rightarrow$ $\mu_{2}$, $\sigma_{2}$
			
			По набору квантилей $\xi_{2}$ находим параметры $\mu_{2}$, $\sigma_{2}$ нормального распределения по формулам (12) и (13).
			\item $\mu_{i}$, $\sigma_{i}$ $\rightarrow$ $m_{i}$, $s_{i}^{2}$
			
			С помощью формул (10) и (11) находим мат. ожидания и дисперсии $\xi_{1}$ и $\xi_{2}.$
			\item $m = m_{1}+m_{2}$
			...
			С помощью формул (10) и (11) находим параметры нормального распределения.
			\item $\mu$, $\sigma$ $\rightarrow$ $z_{\pi}$, $z_{0.5}$, $z_{1-\pi}$
			
			С помощью формулы (13) находим значения квантилей через $\mu$ и $\sigma$.
...
\begin{figure}[!hhh]
	\begin{center}
		\begin{minipage}[h]{0.8\linewidth}
			...
			\caption{Ошибка аппроксимации $q_{90}$ при $\sigma_{1}^{2} = 0.45$.} %% подпись к рисунку
